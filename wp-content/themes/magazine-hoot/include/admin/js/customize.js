...
( function( api ) {
	...
	// Extends our custom "maghoot-premium" section.
...
jQuery(document).ready(function($) {
	...
	var $premiumhead = $('#accordion-section-premium > h3', 'body:not(.maghoot-bcomp)');
	if ( $premiumhead.length ) {
		$premiumhead.prepend('<i class="fa fa-star"></i> ');
		$premiumhead.on( 'click', function(){
			$('body').addClass('maghoot-display-premiumuse');
		});
		...
		$('.customize-section-back').on('click', function(){
			$('body').removeClass('maghoot-display-premiumuse');
		});
	}
