...
function hybridextend_get_excerpt( $words ) {
	if ( empty( $words ) ) {
		...
		$maghoot_theme->excerpt_customlength = $words;
		...
		unset( $maghoot_theme->excerpt_customlength );
		return $return;
	}
}
...
function hybridextend_getexcerpt_customlength( $length ){
	global $maghoot_theme;
	if ( !empty( $maghoot_theme->excerpt_customlength ) )
		return $maghoot_theme->excerpt_customlength;
	else
		return $length;
}
...
function hybridextend_locate_uri( $location, $type, $return = 'uri' ) {
	...
	elseif ( defined( 'HYBRIDEXTEND_DEBUG' ) )
		...
		$loadminified = maghoot_get_mod( 'load_minified', 0 );
...
function hybridextend_get_image_size_name( $span, $crop=NULL, $gridadjust=0 ) {
	...
	// $grid = ( function_exists( 'maghoot_get_mod' ) ) ? intval( maghoot_get_mod( 'site_width' ) ) : 0;
	// if ( empty( $grid ) )
		$grid = $default_grid;
	$grid -= $gridadjust;
