...
do_action( 'maghoot_content_blocks_wrap', 'pages', $content_blocks_query, $page_ids );
?>
...
<div class="content-blocks-widget-wrap content-blocks-pages <?php echo sanitize_html_class( $top_class ); ?>">
	<div class="content-blocks-widget-box <?php echo sanitize_html_class( $bottom_class ); ?>">
		<div class="content-blocks-widget" <?php echo $custommargin; ?>>
			...
			// Template modification Hook
			do_action( 'maghoot_content_blocks_start', 'pages', $content_blocks_query, $page_ids );
			?>
						...
						foreach( $content_blocks_query->posts as $post ) :
							if ( $box['page'] == $post->ID || $altPage == $post->ID ) :
								...
								if ( !empty( $box['icon'] ) ) {
									...
									$img_size = apply_filters( 'maghoot_content_block_img', $img_size, $columns, $style );
									$visual = 1;
								}
								...
								<div class="content-block-column <?php echo 'hcolumn-1-' . $columns; ?> <?php echo sanitize_html_class( 'content-block-' . $style ) . ' ' . $column_class; ?>">
									...
									<?php if ( $linktag ) : ?>
										<?php
										$linktext = ( !empty( $box['link'] ) ) ? $box['link'] : maghoot_get_mod('read_more');
			...
			<?php
			// Template modification Hook
			do_action( 'maghoot_content_blocks_end', 'pages', $content_blocks_query, $page_ids );
			?>
