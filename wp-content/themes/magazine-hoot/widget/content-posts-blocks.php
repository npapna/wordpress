<?php
// Get border classes
$top_class = maghoot_widget_border_class( $border, 0, 'topborder-');
$bottom_class = maghoot_widget_border_class( $border, 1, 'bottomborder-');

// Get custom style attribute
$mt = ( !isset( $customcss['mt'] ) ) ? '' : $customcss['mt'];
$mb = ( !isset( $customcss['mb'] ) ) ? '' : $customcss['mb'];
$custommargin = maghoot_widget_margin_style( $mt, $mb );
...
do_action( 'maghoot_content_blocks_wrap', 'posts', $content_blocks_query, $query_args );
?>
...
<div class="content-blocks-widget-wrap content-blocks-posts <?php echo sanitize_html_class( $top_class ); ?>">
	<div class="content-blocks-widget-box <?php echo sanitize_html_class( $bottom_class ); ?>">
		<div class="content-blocks-widget" <?php echo $custommargin; ?>>
			...
			/* Display Title */
			if ( $title )
				echo wp_kses_post( apply_filters( 'maghoot_content_blocks_title', $before_title . $title . $after_title, $title, $before_title, $after_title ) );

			// Template modification Hook
			do_action( 'maghoot_content_blocks_start', 'posts', $content_blocks_query, $query_args );
			?>
				...
				<?php
						...
						foreach ( $content_blocks_query as $post ) :
								...
								if ( has_post_thumbnail() ) {
									...
									$img_size = maghoot_thumbnail_size( 'column-1-' . $img_size );
									$img_size = apply_filters( 'maghoot_content_posts_block_img', $img_size, $columns, $style );
									$visual = 1;
								}
								...
								<div class="content-block-column <?php echo 'hcolumn-1-' . $columns; ?> <?php echo sanitize_html_class( 'content-block-' . $style ) . ' ' . $column_class; ?>">
									<div class="content-block <?php echo $block_class; ?>">
										...
										<?php if ( $visualtype == 'image' ) : ?>
											<div class="content-block-visual content-block-image">
												<?php echo $linktag;
												maghoot_post_thumbnail( 'content-block-img', $img_size );
												echo $linktagend; ?>
											</div>
										<?php endif; ?>
										...
										<div class="content-block-content<?php
											...
											if ( maghoot_meta_info_display( $metadisplay, $metacontext, true ) ) {
												echo '<div class="content-block-subtitle small">';
												maghoot_meta_info_blocks( $metadisplay, $metacontext );
												echo '</div>';
											} ?>
											<?php
									...
									<?php if ( $fullcontent === 'excerpt' ) : ?>
										<?php
										$linktext = maghoot_get_mod('read_more');
										$linktext = ( empty( $linktext ) ) ? sprintf( __( 'Read More %s', 'magazine-hoot' ), '&rarr;' ) : $linktext;
			...
			<?php
			// Template modification Hook
			do_action( 'maghoot_content_blocks_end', 'posts', $content_blocks_query, $query_args );
			?>
