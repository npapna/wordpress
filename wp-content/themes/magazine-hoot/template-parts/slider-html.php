<?php
global $maghoot_theme, $hybridextend_style_builder;

if ( !isset( $maghoot_theme->slider ) || empty( $maghoot_theme->slider ) )
	return;

// Ok, so we have a slider to show. Now, lets display the slider.

/* Let developers alter slider via global $maghoot_theme */
do_action( 'maghoot_slider_start', 'html' );

/* Create Data attributes for javascript settings for this slider */
$atts = $class = $gridstyle = '';
if ( isset( $maghoot_theme->sliderSettings ) && is_array( $maghoot_theme->sliderSettings ) ) {

	if ( !empty( $maghoot_theme->sliderSettings['class'] ) )
		$class .= ' ' . sanitize_html_class( $maghoot_theme->sliderSettings['class'] );

	if ( !empty( $maghoot_theme->sliderSettings['id'] ) )
		$atts .= ' id="' . sanitize_html_class( $maghoot_theme->sliderSettings['id'] ) . '"';
	foreach ( $maghoot_theme->sliderSettings as $setting => $value )
		$atts .= ' data-' . $setting . '="' . esc_attr( $value ) . '"';

	if ( !empty( $maghoot_theme->sliderSettings['min_height'] ) ) {
		// use height instead of min-height (firefox) http://stackoverflow.com/questions/7790222/
		$gridstylearray = $hybridextend_style_builder->css_rule_sanitized_array( 'height', $maghoot_theme->sliderSettings['min_height'] . 'px' );
...
<ul class="lightSlider<?php echo $class; ?>"<?php echo $atts; ?>><?php
	foreach ( $maghoot_theme->slider as $slide ) :
		...
		$slide['button'] = empty( $slide['button'] ) ? __('Know More', 'magazine-hoot') : $slide['button'];
				...
				<div class="hgrid"<?php if ( !empty( $gridstyle ) ) echo ' style="' . esc_attr( $gridstyle ) . '"'; ?>>
					...
					<?php if ( !empty( $slide['image'] ) ) { ?>
						<div class="<?php echo $column; ?> hootslider-html-slide-column hootslider-html-slide-right">
							...
							if ( !empty( $imageid ) )
								echo wp_get_attachment_image( $imageid, apply_filters( 'maghoot_htmlslider_imgsize', 'full', '', array( 'class' => 'hootslider-html-slide-img' ) ) );
							else
