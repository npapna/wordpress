...
do_action( 'maghoot_slider_start', 'image' );
...
if ( isset( $maghoot_theme->sliderSettings ) && is_array( $maghoot_theme->sliderSettings ) ) {
	if ( isset( $maghoot_theme->sliderSettings['class'] ) )
		$class .= ' ' . sanitize_html_class( $maghoot_theme->sliderSettings['class'] );
	if ( isset( $maghoot_theme->sliderSettings['id'] ) )
		...
		if ( !empty( $slide['image'] ) ) :
			...
			?><li class="lightSlide hootslider-image-slide hootslider-image-slide-<?php echo $slide_count; $slide_count++; ?>">
				...
				<?php if ( !empty( $slide['url'] ) && empty( $slide['button'] ) )
					...
					if ( !empty( $imageid ) )
						echo wp_get_attachment_image( $imageid, apply_filters( 'maghoot_imageslider_imgsize', 'full' ), '', array( 'class' => 'hootslider-image-slide-img' ) );
					else
