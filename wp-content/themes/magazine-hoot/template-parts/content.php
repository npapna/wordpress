...
?>
	...
	<article <?php hybridextend_attr( 'post' ); ?>>
		...
		if ( !$hide_meta_info && 'bottom' == maghoot_get_mod( 'post_meta_location' ) && !is_attachment() ):
			$metarray = maghoot_get_mod('post_meta');
			if ( maghoot_meta_info_display( $metarray, 'post', true ) ) :
			?>
			<footer class="entry-footer">
				<?php maghoot_meta_info_blocks( $metarray, 'post' ); ?>
			</footer><!-- .entry-footer -->
			<?php
			endif;
		endif;
 ...
 */
	...
	$archive_type = apply_filters( 'maghoot_default_archive_type', 'block2', 'content' );
	$archive_template = apply_filters( 'maghoot_default_archive_location', 'template-parts/content-archive', $archive_type, 'content' );
